...
def test_balanced_region_keeps_all_short_strokes() -> None:
    ...
    # Perpendicular gaps are wider than _TEXT_CHAIN_MAX_GAP so the text
...
def test_h_dominated_cell_drops_short_h_strokes() -> None:
    ...
    # Perpendicular spacing is alternately tiny/huge so the text-chain
    # uniformity check rejects any chain of 3: this isolates the
    # orientation-dominance filter.
    candidates = [
        _h(page=0, y=10, x0=10, x1=50),
        _h(page=0, y=15, x0=10, x1=50),   # gap 5
        _h(page=0, y=100, x0=10, x1=50),  # gap 85
        _h(page=0, y=115, x0=10, x1=50),  # gap 15
    ]
    walls = classify_walls(candidates)
...
def test_long_h_stroke_survives_h_dominated_cell() -> None:
    ...
    # survive even when the cell is H-dominated. Keep gaps alternately
    # tiny/huge so the text chain filter does not fire.
    candidates = [
        _h(page=0, y=10, x0=10, x1=50),
        _h(page=0, y=15, x0=10, x1=50),   # gap 5
        _h(page=0, y=100, x0=10, x1=50),  # gap 85
        _h(page=0, y=115, x0=10, x1=210),  # gap 15, len 200
    ]
    walls = classify_walls(candidates)
    assert len(walls) == 1
    remaining = walls[0]
...
def test_length_exactly_at_threshold_is_protected() -> None:
    ...
    # cell. Alternating tiny/huge gaps keep the text chain filter off.
    candidates = [
...
def test_ratio_exactly_at_threshold_fires() -> None:
    ...
    # 4 short H strokes drop; the lone V stroke stays. Use non-uniform
    # gaps to avoid the text chain filter.
    candidates = [
        _h(page=0, y=10, x0=10, x1=50),
        _h(page=0, y=15, x0=10, x1=50),   # gap 5
        _h(page=0, y=100, x0=10, x1=50),  # gap 85
