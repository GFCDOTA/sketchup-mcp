...
| 2026-05-26 | (cleanup pass 5 — deeper archive prune) | Continued hygiene cycle after PR #182. Deleted 5 more files: (1) `docs/_archive/2026-05-md-cleanup/SOLUTION.md` (20 KB, ZERO active refs; the `2026-05-md-cleanup/` dir is now empty and auto-removed); (2) `docs/_archive/2026-04-f1-cycle/F1-DASHBOARD.html` (49 KB output artifact, regenerable via `scripts/generate_f1_dashboard.py`); (3) `docs/_archive/2026-04-f1-cycle/SVG-INGEST-INTEGRATION.md` (13 KB, only refs were dangling pointers — `patches/archive/README.md` had a broken `../../docs/` path since the 2026-05-04 archive move + my own pass-3 entry in PROJECT_STATE); (4) `docs/_archive/2026-04-f1-cycle/SVG-MAIN-PLAN-ISOLATION.md` (11 KB, same pattern — algorithmic info lives in `tools/build_vector_consensus.py` source); (5) `scripts/render_validation_summary_png.py` (8 KB, orphan F1-cycle render script, only mention was the dashboard architecture_radar example fixture). Updated `patches/archive/README.md` to drop the dangling `SVG-INGEST-INTEGRATION.md` reference (replaced with pointer to `tools/build_vector_consensus.py`) and fix the `SOLUTION-FINAL.md` ref to use the correct archive path. Rewrote `docs/_archive/2026-04-f1-cycle/README.md` table to reflect the 2 surviving load-bearing entries (`OVER-POLYGONIZATION-ANALYSIS.md` + `SOLUTION-FINAL.md`). Net: -5 files, ~100 KB freed. No algorithm/schema/threshold/Ruby/SU/CI touched. F1-cycle archive now contains only files actively referenced by code or canonical docs. |
