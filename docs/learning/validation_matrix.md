...
| **Ruby/SU exporter** (`tools/consume_consensus.rb`, `tools/inspect_walls_report.rb`, autorun plugins, `tools/skp_from_consensus.py`, `.mcp.json`) | Run cheap gates manually (JSON validation, preview PNG, consensus hash) then `tools.skp_from_consensus`; compare `inspect_report.json` before/after; visual inspection of .skp; constants justified. Smoke harness `scripts/smoke/smoke_skp_export.py` planned (`tooling/sketchup-smoke-gates`). | sketchup-specialist |
