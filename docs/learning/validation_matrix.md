...
| **Ruby/SU exporter** (`tools/consume_consensus.rb`, `tools/inspect_walls_report.rb`, autorun plugins, `tools/skp_from_consensus.py`, `.mcp.json`) | Run `scripts/smoke/smoke_skp_export.py`; compare `inspect_report.json` before/after; visual inspection of .skp; constants justified. | sketchup-specialist |
