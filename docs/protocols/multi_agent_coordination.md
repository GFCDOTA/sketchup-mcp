...
### 5. Use an isolated `git worktree` for shared-working-tree work
When working in a directory that another agent may be using:
```bash
git worktree add -b chore/<scoped-name> <isolated-path> origin/develop
cd <isolated-path>
# do work
cd -
git worktree remove <isolated-path>
```
The cost is ~2 seconds; the benefit is that your branch switches,
stashes, and uncommitted edits never collide with a peer agent's
working state in the same repo.
...
- `docs/learning/lessons_learned.md` LL-019 — the lesson and
  incident timeline that motivated this protocol.
- `CLAUDE.md` §22 — short summary of the rule (this doc is the
  long form).
- `CLAUDE.md` §0 — git flow (PRs always against `develop`).
- `CLAUDE.md` §4 — PR standard.
- `docs/protocols/terminal_first_github_auth.md` — companion
  protocol; covers HOW to call GitHub safely after this protocol
  determines WHAT is safe to call.
- `.ai_bridge/HANDOFF.md` — the public coordination channel.
