...
> [`HANDOFF.md`](HANDOFF.md), [`PR_HYGIENE.md`](PR_HYGIENE.md)
> (startup/exit + terminal-first GitHub workflow),
> [`AGENT_COORDINATION.md`](AGENT_COORDINATION.md) (worktree isolation
> for multi-agent sessions — the architectural precondition for safe
> shared-repo cleanup),
> [`ARTIFACT_POLICY.md`](ARTIFACT_POLICY.md) (`.skp` is the primary
> deliverable; promote out of `runs/` into `artifacts/human_review/`
> whenever a PR claims SKP success),
> [`../CLAUDE.md`](../CLAUDE.md) §15.
> **Per-cycle audit reports:** under `reports/current/` (e.g.,
> [`../reports/current/repo_health_report.md`](../reports/current/repo_health_report.md)
> generated by `tools/repo_health_gate.py`) and `docs/ops/` (e.g.,
> `docs/ops/repo_hygiene_audit_2026-05-10.md`).
> **Automated enforcement:** `tools/repo_health_gate.py` (3 modes:
> audit / check / fix) + `scripts/project_state_check.py`. CI:
> [`../.github/workflows/repo_health.yml`](../.github/workflows/repo_health.yml).
> Detector + safe-fix catalogue: top of `tools/repo_health_gate.py`.
...
4. Run `python tools/repo_health_gate.py --mode audit` — get the
   current finding list (`reports/current/repo_health_report.md`).
5. If safe fixes are available, optionally run
   `python tools/repo_health_gate.py --mode fix --dry-run` to
   preview, then drop `--dry-run` to apply (moves old reports,
   tightens `.gitignore`, deletes untracked tmp files).
6. If a deletion or archive is needed → open a SEPARATE `chore:`
