"""Renderiza observed_model.json em PNG. Uso: ``python -m renderers.debug <run_dir>``.

Migrated from repo root ``render_debug.py`` (2026-05-08) per
``docs/architecture/target_repo_architecture.md`` step 5. The original
root entry point is preserved as a deprecation wrapper.
"""
import json
import os
import sys
...
def render(run_dir: Path) -> Path:
    """Render the full debug overlay into ``<run_dir>/debug_combined.png``.
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_dir = Path(args[0]) if args else Path("runs/test_plan")
    render(run_dir)
    return 0
