# Handoff — 2026-05-25 (post #177 — orphan SU diagnostic tools tracked; PR #176 + #177 shipped this turn)
...
| Worktree | `E:\Claude\sketchup-mcp` (canonical clone) |
| Branch at session start | `fix/build-plan-shell-runner-mode` (already-merged feature branch, PR #155) with 4 untracked SU tools sitting in the work tree |
| Branch at session end | `develop` (clean, fast-forwarded to `3004180` post #177 merge) |

## What landed this turn

| PR  | Title | Merge commit | Notes |
|---|---|---|---|
| **#176** | feat(fixture): quadrado uniform 4-windows — fidelity reference (JSON-only, SKP deferred) | `a29c9dc` | Squash + branch deleted. Pre-existing PR opened by Felipe; all CI green at session start so this session merged it. |
| **#177** | chore(tools): track orphan SU diagnostic tools (build_room_ring + dump_skp_groups) | `3004180` | Branch `chore/track-orphan-su-diagnostic-tools`. +727 LOC across 4 files. First push failed `G-REPO-HEALTH` E006 ("structural diff under `tools/` requires PROJECT_STATE.md or HANDOFF.md update"); fixed by adding a row to `docs/PROJECT_STATE.md` §9 in a follow-up commit, then merged. |

`develop` HEAD: `3004180`.

## Why PR #177 existed at all

The 4 files (`tools/build_room_ring_skp.{py,rb}` +
`tools/dump_skp_groups.{py,rb}`) had been sitting **untracked** in the
work tree despite already being cited as live tooling in committed
docs:

- `CONTRIBUTING.md` "Live debugging" section listed
  `tools/dump_skp_groups.{py,rb}` as a non-disruptive geometry dumper.
- `docs/learning/failure_patterns.md` FP-014 referenced
  `tools/build_room_ring_skp.py` as a launcher that uses the
  `disarm_sketchup_autoruns` helper in its try/finally.
- `docs/adr/ADR-003-plan-shell-exporter.md` §1 credits
  `tools/build_room_ring_skp.{py,rb}` as the single-room
  face-with-hole + one-pushpull precursor that scaled into
  `build_plan_shell_skp.{py,rb}`.

`git log --all -- <those 4 files>` returned **empty** — they had
never been tracked in any branch. This session closed the docs/reality
mismatch.

`build_room_ring_skp.rb` does call `model.entities.clear!`. That is
intentional per ADR-003 §1 (it IS the single-room rebuild precursor);
the in-place-edit rule in `feedback_nao_rebuild_edite_correto` applies
specifically to `consume_consensus.rb`, which this PR did not touch.

## Operational notes for next session

1. **Python interpreter:** the MS-Store `python.exe` stub at
   `%LOCALAPPDATA%\Microsoft\WindowsApps\python.exe` points at a
   missing `Python312` install (path `%LOCALAPPDATA%\Programs\Python\
   Python312\python.exe` is gone). When running gate scripts locally,
   use `E:\Claude\sketchup-mcp\.venv\Scripts\python.exe` (repo venv,
   works) or reinstall Python 3.12. Confirmed this turn by running
   `repo_health_gate` locally against PR #177.
2. **`reports/current/repo_health_report.md`** is regenerated by
   `repo_health_gate.py` when run without `--no-report`. The CI
   workflow writes it only to its artefact bundle; the tracked
   on-disk copy under `reports/current/` should be regenerated
   deliberately as part of a snapshot commit, not silently
   touched by side-effect runs. This session discarded a local
   side-effect mutation rather than commit a stale-base snapshot.
3. **PR #177 explicitly deferred test coverage** for the two
   newly-tracked tools (both require SU 2026 GUI per CLAUDE.md
   §3 + §18). The natural follow-up is a CI-safe smoke harness
   (env-detect SU 2026, skip cleanly when absent — same pattern
   as `scripts/smoke/smoke_skp_export.py`).
4. **Stop policy from #173 still active for Slice 6c.** Do not
   start `manual_polygon_room_count` work without explicit user
   trigger.

## Pipeline state for planta_74

Unchanged from 2026-05-13 baseline. 4-axis verdict still WARN
(advisory `global_visual_fidelity`); issue #122 tracks the visual
